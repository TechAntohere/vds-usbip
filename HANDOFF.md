...
Complete for anything real software does: byte-exact descriptors; FULL input report (buttons/sticks/triggers/gyro/accel/touchpad/timestamp); output (rumble/lightbar/trigger FX/player LED/mute LED); audio out+in+haptics+volume; feature reports incl. firmware/factory (0x20). Command-channel diagnostics partly work after the cache fix (commit 020cc63): 0x80/0x81 (and 0x84/0x85) are dynamic command req/resp and must NOT be cached (was caching 0x81 → stale → daidr factory/diagnostic fields blank). Now command-channel (>=0x80) always reads fresh from BT. Result: "some, not all" daidr factory fields populate. CORRECTION (user): the physical controller DOES expose all of them over Bluetooth (daidr works fully over direct BT), so the remaining blanks are a vds COMMAND-PROTOCOL FORWARDING GAP, not a hardware limit — likely the 0x80→0x81 request/response sequencing, per-command timing (our feature-GET waits only 500ms and daidr polls 0x81 repeatedly), or BT feature framing/CRC through the bridge. Read-only, zero gameplay impact → low priority, but it's a fixable vds bug, not "unsupported." Worth revisiting if full factory/diagnostic fidelity is ever wanted.

## DONE: audio (speaker+haptics) latency — ✅ user-verified "better, and clean" (2026-07-15). Latency ~240ms→~60ms, no crackle. Env-tunable (VDS_AUDIO_PRIME_CHUNKS, try 3-4 for even lower if wanted). Details below.
User reported in-game audio+haptic latency too high (haptics ride in the same BT 0x36 report as speaker, so they share latency). Root cause: audio jitter buffers were sized huge (kAudioPrimeChunks=24=240ms steady-state, kAudioFreshQueueChunks=48=480ms cap, kAudioJitterBufferMaxDelay=300ms) — tuned back when Windows over-delivered audio ~5x real time (pre-pacing). Now ISO OUT is paced to real time so production≈consumption; buffers can shrink a lot. CHANGE MADE (built, NOT yet tested by user — they had to step away): kAudioPrimeChunks/kAudioFreshQueueChunks are now env-tunable (VDS_AUDIO_PRIME_CHUNKS default 6=~60ms, VDS_AUDIO_QUEUE_CHUNKS default 12=~120ms cap); kMaxPendingAudioChunks + kAudioHighWatermarkChunks derive from them; kAudioJitterBufferMaxDelay 300→120ms. Committed (pending user test) so work isn't lost. NEXT SESSION: test in-game — is latency better AND still crackle-free? Dial VDS_AUDIO_PRIME_CHUNKS lower (3-4) if latency still high & clean, or raise if crackle returns (set env in run_vdsd.ps1 or a run_vdsd_gain-style launcher). Daemon currently running with defaults (prime=6/queue=12), attached.

## PARKED: data/control layer for UI (task #12) — partial scaffolding in vdsd.cc
Started before the latency pivot, then parked (harmless, compiles): added `struct WorkerStatus` (atomics: battery/charge/headphone_jack/mic_detected/mic_muted/mic_active/speaker_active/polling_hz) after BridgeState (~line 400), `publish_input_status()` helper (parses battery/jack/mic from USB input report — offsets from daidr: payload byte 52=status0 battery low-nibble×10 + charge high-nibble, byte 53=status1 bit0 jack/bit1 mic), and a `WorkerStatus status;` member on BridgeWorker. NOT yet wired: threading WorkerStatus* into run_bridge_session→handlers, calling publish_input_status, computing polling_hz, the `status` vdsctl command + JSON, and setters. Plan in "UI plan" section below. Stack decided: C#/.NET WPF (.NET 10 present). daidr input-report offsets confirmed. Suggestion given: interface via `vdsctl status`(JSON)/`vdsctl set`; defer color (needs command-protocol forwarding fix); report live measured Hz.

## UI (tray app) — IN PROGRESS
Data layer (read side) DONE + committed (b1b039e): `vdsctl status` returns live JSON per controller (battery%, charge_status, headphone_jack, mic_detected/muted/active, speaker_active, polling_hz, has_input). Parsed from DualSense input report (payload byte52=battery/charge, byte53 bit0 jack/bit1 mic) + rolling-window rate. Verified: battery 90, polling ~540Hz.

WPF tray app DONE (foundation, commit f35aa7c) at `ui/VdsTray/` (C#/.NET 10, `dotnet build -c Release`):
- Single-manager: ensures vdsd running + attached on launch; tray icon (LEFT-click = show status popup, RIGHT-click = menu); menu has Start-on-boot (Run key), Reconnect, Exit.
- Polls `vdsctl status` every 1s. Bottom-right connection card (bg #272727), fades after 5s: "Dualsense" title (28px), Connected + battery pill (green/amber/red), controller art from assets/dualsense.png, right column headphone-jack/mic/polling-Hz. Mini toast for jack/mic changes.
