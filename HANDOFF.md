...
| Mic capture quality | 🔶 NEXT | User: "crackling, not audible." Log sample showed peak_ch0/ch1=0 (silence) at len=1920 (10ms chunks streaming continuously). Enable path is IDENTICAL to working Linux (shared vds_protocol.cc; defaults mic_muted=false/headset_mic_plugged=false match). So NOT an enable-logic bug. Two things to chase next session: (1) mic-side jitter buffer — take_mic_bytes zero-pads on every underrun (Windows pulls isoc IN every 1ms fixed-size; BT mic arrives in 10ms bursts) = gap crackle, the IN-side mirror of the speaker pacing problem; add a prefill/watermark before returning real bytes. (2) Confirm real samples: sample was sparse (count%1000 log); densify peak logging or have user speak while watching a denser trace to tell silence(enable) from gaps(jitter). DualSenseBtProbe has a working live mic-in reference if enable turns out wrong. |
