...
function Get-VdsSemverParts {
    ...
    if ($TagName -notmatch "^\d+\.\d+\.\d+(\.\d+)?$") {
        return $null
    }

    $Values = @()
    foreach ($Part in ($TagName -split "\.")) {
        [int]$Value = 0
...
function Get-VdsDriverVersionFromGit {
    ...
    try {
        ...
        return Convert-VdsTagToDriverVersion -TagName $BestTag -Distance $BestDistance
    } catch {
        return $null
    }
}
